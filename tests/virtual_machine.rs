...
mod vm_tests {
    ...
    #[tokio::test]
    async fn test_all_threads_cmd() {
        let mock_stream = MockStreamBuilder::default()
            .response_bytes(
                &[
                    // no out data, length=0x1c, id=0x2, cmd: (0x1 << 8) | 0x4
                    0x0, 0x0, 0x0, 0xb, 0x0, 0x0, 0x0, 0x2, 0x0, 0x1, 0x4,
                ],
                &[
                    // reply, length=0x3f, id=0x2
                    // six threads: threadID(1), threadID(2), threadID(3), threadID(4),
                    // threadID(5), threadID(6)
                    0x0, 0x0, 0x0, 0x3f, 0x0, 0x0, 0x0, 0x2, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x6,
                    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2,
                    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4,
                    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x6,
                ],
            )
            .build();
        let client = JdwpClient::new(mock_stream).await.unwrap();
        let reply = client.vm_get_all_threads().await.unwrap();
        assert_eq!(reply.threads.len(), 6, "Thread count mismatch");
        for i in 1..reply.threads.len() + 1 {
            let thread = reply.threads[i - 1];
